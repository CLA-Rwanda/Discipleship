...
import { KeyRound, Eye, EyeOff, ShieldCheck, Skull, AlertTriangle } from "lucide-react";
...
export default function SettingsPage() {
  ...
  async function handleEraseAll() {
    ...
    if (result.success) {
      ...
      setEraseError(result.error ?? "Something went wrong.");
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in max-w-lg">
      ...
      {isSuperAdmin && (
        <div
          className="rounded-xl overflow-hidden"
          style={{ border: "2px solid rgba(192,40,40,0.5)" }}
        >
          {/* Header */}
          <div
            className="px-5 py-4 flex items-center gap-3"
            style={{ background: "rgba(192,40,40,0.12)", borderBottom: "1px solid rgba(192,40,40,0.3)" }}
          >
            <Skull size={20} style={{ color: "#ff4444" }} />
            <div>
              <h2 className="text-lg font-extrabold" style={{ fontFamily: "Barlow Condensed, sans-serif", color: "#ff4444" }}>
                Danger Zone
              </h2>
          ...
          <div className="p-5 flex flex-col gap-4" style={{ background: "rgba(139,26,26,0.07)" }}>
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                ...
                <p className="text-sm mt-0.5" style={{ color: "rgba(248,240,230,0.45)" }}>
                  Permanently deletes every registered member and all attendance records. Classes and facilitators are not affected.
                </p>
              </div>
              {!eraseSuccess && dangerStep === 0 && (
                <button
                  ...
                  style={{
                    ...
                    color: "#ff4444",
                    border: "1px solid rgba(192,40,40,0.5)",
                  }}
                >
                  Erase All Data
                </button>
            ...
            {dangerStep === 1 && eraseCounts && (
              <div
                ...
                style={{ background: "rgba(192,40,40,0.12)", border: "1px solid rgba(192,40,40,0.4)" }}
              >
                <div className="flex items-start gap-3">
                  <AlertTriangle size={20} style={{ color: "#ff4444", flexShrink: 0, marginTop: 2 }} />
                  <div className="flex flex-col gap-1">
                    <p className="font-extrabold text-base" style={{ color: "#ff4444", fontFamily: "Barlow Condensed, sans-serif" }}>
                      YOU ARE ABOUT TO PERMANENTLY DELETE:
                    </p>
                    <ul className="text-sm flex flex-col gap-0.5 mt-1" style={{ color: "rgba(248,240,230,0.8)" }}>
                      <li>
                        <span className="font-bold" style={{ color: "#ff6b6b" }}>{eraseCounts.members.toLocaleString()} registered member{eraseCounts.members !== 1 ? "s" : ""}</span>
                      </li>
                      <li>
                        <span className="font-bold" style={{ color: "#ff6b6b" }}>{eraseCounts.attendance.toLocaleString()} attendance record{eraseCounts.attendance !== 1 ? "s" : ""}</span>
                      </li>
                    </ul>
                    <p className="text-sm mt-2 font-bold" style={{ color: "#ff4444" }}>
                      THIS CANNOT BE UNDONE. There is no backup. The data will be gone forever.
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
                  <button
                    onClick={() => setDangerStep(0)}
                    className="flex-1 py-2 rounded-lg text-sm font-bold"
                    style={{ border: "1px solid rgba(255,255,255,0.15)", color: "rgba(248,240,230,0.6)" }}
                  >
                  ...
                  <button
                    ...
                    className="flex-1 py-2 rounded-lg text-sm font-bold"
                    style={{ background: "rgba(192,40,40,0.3)", color: "#ff4444", border: "1px solid rgba(192,40,40,0.5)" }}
                  >
            ...
            {dangerStep === 2 && (
              <div
                className="flex flex-col gap-4 p-4 rounded-xl"
                style={{ background: "rgba(192,40,40,0.15)", border: "2px solid rgba(192,40,40,0.6)" }}
              >
                <div className="flex items-center gap-2">
                  <Skull size={18} style={{ color: "#ff4444" }} />
                  <p className="font-extrabold" style={{ color: "#ff4444", fontFamily: "Barlow Condensed, sans-serif", fontSize: "1rem" }}>
                    FINAL CONFIRMATION REQUIRED
                  </p>
                </div>
                <p className="text-sm" style={{ color: "rgba(248,240,230,0.7)" }}>
                  Type <span className="font-extrabold" style={{ color: "#ff4444" }}>ERASE</span> in the box below to confirm you want to permanently destroy all member and attendance data.
                </p>
                <input
                  type="text"
                  value={confirmInput}
                  ...
                  className="cla-input font-bold tracking-widest text-center"
                  style={{ borderColor: confirmInput === "ERASE" ? "#ff4444" : undefined, color: "#ff6b6b" }}
                  autoComplete="off"
                />
                {eraseError && (
                  <p className="text-sm" style={{ color: "#ff6b6b" }}>{eraseError}</p>
                )}
                <div className="flex gap-3">
                  <button
                    onClick={() => { setDangerStep(0); setConfirmInput(""); }}
                    className="flex-1 py-2 rounded-lg text-sm font-bold"
                    style={{ border: "1px solid rgba(255,255,255,0.15)", color: "rgba(248,240,230,0.6)" }}
                  >
                    Cancel
                  ...
                  <button
                    ...
                    disabled={confirmInput !== "ERASE" || erasing}
                    className="flex-1 py-2.5 rounded-lg text-sm font-extrabold transition-all"
                    style={{
                      fontFamily: "Barlow Condensed, sans-serif",
                      fontSize: "0.95rem",
                      letterSpacing: "0.04em",
                      background: confirmInput === "ERASE" ? "#8b1a1a" : "rgba(192,40,40,0.1)",
                      color: confirmInput === "ERASE" ? "#ffffff" : "rgba(255,100,100,0.3)",
                      border: "1px solid rgba(192,40,40,0.5)",
                      cursor: confirmInput !== "ERASE" ? "not-allowed" : "pointer",
                    }}
                  >
